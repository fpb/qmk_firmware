...
### ChibiOS submodule patches (all three required)

This branch needs **three** working-tree patches applied to `lib/chibios-contrib` before building. They touch disjoint files, so order does not matter:
...
Note: all three are working-tree edits of the `lib/chibios-contrib` submodule and are discarded by `git submodule update`; re-apply if that happens.
