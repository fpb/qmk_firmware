...
// idle-sleep call is a no-op and the effect keeps animating. The kb drives the
// state manually (see kb_sleep_task in ak820pro.c).
...
// Idle sleep: after this many ms with no key/encoder input, blank the LCD
// (display-off + backlight off) and the RGB matrix; any input wakes them. The
// MCU keeps scanning, so the waking key still types. 0 disables the timer (a real
// USB bus-suspend still sleeps, on hosts that suspend -- see ak820pro.c).
