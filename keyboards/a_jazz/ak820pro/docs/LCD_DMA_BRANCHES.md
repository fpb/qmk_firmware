...
| `ak820pro-flashlcd-unified-dualspi` / `-unified-dualspi-qp` | The standalone **custom-only** and **QP-only** builds, now folded into `-dualspi-dual` as its two backends. Kept as the individually battle-tested variants. |
| `ak820pro-flashlcd-tiles` | The **leanest** build: the custom tile dashboard fully bare-metal — owns SPI0 directly, no `HAL_USE_SPI`, no ChibiOS SPI dependency (~0.5 KB smaller). |
